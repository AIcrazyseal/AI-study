...
特点：决策核心由工程师明确设计的“条件-动作”规则构成，如图 1.2 所示。\
优点：结构最简单的 ，可靠且高效。缺点：完全依赖于当前的感知输入，不具备记忆或预测能力。无法应对需要理解上下文的复杂任务。
...
特点：拥有一个内部的 **世界模型（World Model），** 用于追踪和理解环境中那些无法被直接感知的方面,试图回答：“世界现在是什么样子的？”。\
优点：这个内部模型让智能体拥有了初级的“记忆”，使其决策不再仅仅依赖于瞬时感知，而是基于一个更连贯、更完整的世界状态理解。
* **基于目标的智能体（Goal-Based Agent）** 单目标达成
特点：主动地、有预见性地选择能够导向某个特定未来状态的行动。需要回答的问题是：“我应该做什么才能达成目标？”。经典的例子是 GPS 导航系统：你的目标是到达公司，智能体会基于地图数据（世界模型），通过搜索算法（如 A*算法）来规划（Planning）出一条最优路径。\
优点：核心能力体现在了对未来的考量与规划上。
* **基于效用的智能体（Utility-Based Agent）:** 多目标权衡最优
特点：为每一个需要达成的目标都赋予一个效用值，这个值代表了目标实现的满意度高低。它需要回答“哪种行为能为我带来最满意的结果？”。\
优点：多目标权衡。智能体学会在相互冲突的多个目标之间进行权衡，使其决策更接近人类的理性选择。
