...
- 架构设计核心原则：分层解耦、职责单一、接口统一
- **先体验，后实现**的学习路径：直接使用 `pip install "hello-agents==0.1.1"`安装框架，运行示例代码，快速体验各种功能
- 深度学习：从零开始实现每个组件，深入理解框架的设计思想和实现细节

## 7.2 HelloAgentsLLM扩展

把基础客户端改造为一个更具适应性的模型调用中枢。升级目标：

- **云端多提供商支持**：实现对 OpenAI、ModelScope、智谱 AI 等多种主流 LLM 服务商的无缝切换，避免框架与特定供应商绑定。
- **本地模型集成**：引入 VLLM 和 Ollama 这两种高性能本地部署方案，满足数据隐私和成本控制的需求。
- **自动检测机制**：建立一套自动识别机制，使框架能根据环境信息智能推断所使用的 LLM 服务类型，简化用户配置过程。

### 7.2.1 支持多提供商

在实际应用中，不同的服务商在环境变量命名、默认 API 地址和推荐模型等方面都存在差异。如果每次切换服务商都需要用户手动查询并修改代码，会极大影响开发效率。为了解决这一问题，我们引入 `provider`， 在内部处理不同服务商的配置细节，为用户提供一个统一、简洁的调用体验。

通过继承 `HelloAgentsLLM`，来增加对 ModelScope 平台的支持。**学会使用、扩展(继承并扩展第三方库的基类)框架**，不推荐直接修改已安装的库源码，因为不利于库的后续版本升级。
在不修改第三方库源码的前提下，成功为其扩展了新的功能。这种方法不仅保证了代码的整洁和可维护性，也使得未来升级 `hello-agents` 库时，我们的定制化功能不会丢失。

- （1）创建自定义LLM类并继承
  - 从第三方库中导入 `AgentsLLM` 基类；
  - 创建一个名为 `MyLLM`  LLM客户端 的新类继承它。
- （2）重写 `__init__` 方法以支持新供应商
  - 在 `MyLLM`  LLM客户端类中重写 `__init__` 方法。当用户传入 `provider="modelscope"` 时，执行我们自定义的逻辑；
  - 否则，就通过 `super().__init__(...)`调用父类 `AgentsLLM` 的原始逻辑，保留了原有框架的全部功能，使其能够继续支持 OpenAI 等其他内置的供应商。
- （3）使用自定义的 `MyLLM` 类:在项目的业务逻辑中使用我们自己的 `MyLLM`LLM客户端类。
  - 在 `.env` 文件中配置 ModelScope 的 API 密钥：
  - 在主程序中导入并使用 `MyLLM`：`from my_llm import MyLLM # 注意:这里导入我们自己的类`
...
VLLM 是一个为 LLM 推理设计的高性能 Python 库。它通过 PagedAttention 等先进技术，可以实现比标准 Transformers 实现高出数倍的吞吐量。本地部署VLLM服务的完整步骤：

- 安装 VLLM。推荐遵循其[官方文档](https://docs.vllm.ai/en/latest/getting_started/installation.html)进行安装，以避免版本不匹配问题。
  > pip install vllm
  >
- 启动 VLLM 服务，使用以下命令即可启动一个兼容 OpenAI 的 API 服务。VLLM 会自动从 Hugging Face Hub 下载指定的模型权重（如果本地不存在）,并加载 Qwen1.5-0.5B-Chat 模型。
  ```
...
- 下载、安装Ollama [官方网站](https://ollama.com)。
- Ollama下载模型、服务封装和硬件加速配置。`ollama run llama3`
- 访问Ollama： 默认会在 `http://localhost:11434/v1` 地址上暴露 OpenAI 兼容的 API 接口。

#### 7.2.2.3 **接入 `HelloAgentsLLM`**

VLLM 和 Ollama 都遵循了行业标准 API。我们只需在实例化客户端时，将它们视为一个新的 `provider` 即可。

智能体核心代码无需任何修改，就可以在云端 API 和本地模型之间自由切换。
...
为减少用户的配置负担并遵循“约定优于配置”的原则，`HelloAgentsLLM` 内部设计了两个核心辅助方法协同工作：

- `_auto_detect_provider` 负责根据环境信息推断LLM服务商
- `_resolve_credentials` 则根据推断结果完成具体LLM的参数配置。

#### 7.2.3.1 `_auto_detect_provider`
...
#### 7.2.3.2 `_resolve_credentials` 方法

`_resolve_credentials` 方法会根据 `provider` 的值，主动查找对应的环境变量，并为其设置默认的 `base_url`。

相比于4.1.3节的基础实现，现在的HelloAgentsLLM具有以下显著优势：

<div align="center">
  <p>表 7.1 HelloAgentLLM不同版本特性对比</p>
  <img src="https://raw.githubusercontent.com/datawhalechina/Hello-Agents/main/docs/images/7-figures/table-01.png" alt="" width="70%"/>
</div>
...
- 将配置项按逻辑划分为 `LLM配置`、`系统配置` 等，使结构一目了然。
- 每个配置项都设有合理的默认值，保证了框架在零配置下也能工作。
- `from_env()` 类方法，它允许用户通过设置环境变量来覆盖默认配置，无需修改代码，在部署到不同环境时尤其有用。

### 7.3.3 Agent 抽象基类

`Agent` 类是整个框架的顶层抽象。它定义了一个智能体应该具备的通用行为和属性，但并不关心具体的实现方式。通过 Python 的 `abc` (Abstract Base Classes) 模块来实现它，这强制所有具体的智能体实现（如后续章节的 `SimpleAgent`, `ReActAgent` 等）都必须遵循同一个“接口”。

- Agent抽象基类通过继承 `ABC` 被定义为一个不能直接实例化的抽象类。
- 构造函数 `__init__` 清晰地定义了 Agent 的核心依赖：名称、LLM 实例、系统提示词和配置。
- 使用 `@abstractmethod` 装饰的 `run()`，**强制所有子类必须实现 `run()`**，从而保证了所有智能体都有统一的执行入口。
- **扩展性**：开发者可以轻松创建新的智能体子类，只需实现 run 方法即可融入现有框架。
- 基类还提供了通用的历史记录管理方法，这些方法与 `Message` 类协同工作，体现了组件间的联系。

## 7.4 Agent范式的框架化实现

本节内容将把经典Agent范式（SimpleAgent、ReAct、Plan-and-Solve、Reflection）这些独立的Agent实现，改造为基于统一架构的框架组件。重构目标：

1. **提示词工程的系统性提升**：对提示词进行深度优化，从特定任务导向转向通用化设计，增强格式约束和角色定义。
2. **接口与格式的标准化统一**：建立统一的Agent基类和标准化的运行接口，所有Agent都遵循相同的初始化参数、方法签名和历史管理机制。
3. **高度可配置的自定义能力**：支持用户自定义提示词模板、配置参数和执行策略。

### 7.4.1 SimpleAgent

SimpleAgent是最基础的Agent实现，它展示了如何在框架基础上构建一个完整的对话智能体。通过继承框架基类来重写SimpleAgent。首先，在你的项目目录中创建一个 `my_simple_agent.py`文件：
